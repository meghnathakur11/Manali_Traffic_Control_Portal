  ...
  addBookingPassData = async (
    ...
    try {
      ...
      if (payload && token) {
        ...
        // keep this number in the env
        if (
          Number(locationBookingCount) >=
          Number(process.env.MAX_BOOKING_COUNT_PER_LOCATION)
        ) {
          return {
            error: true,
            data: null,
            ...
            message:
              'Bookings are full for the particular date and location,please choose another date or location'
          };
        }
        ...
        if (isBookingPresent) {
          return {
            ...
            status: STATUS_CODES.BADREQUEST,
            message:
              'User booking already present for the same date, location, and car number'
          };
        }
